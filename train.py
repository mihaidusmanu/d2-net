...
parser.add_argument('--scene_info_path', type=str, required=True,
...
parser.add_argument('--checkpoint_prefix', type=str, default='d2',
...
if args.use_validation:
    ...
    validation_dataloader = DataLoader(validation_dataset, batch_size=args.batch_size, num_workers=args.num_workers)
...
def process_epoch(epoch_idx, model, loss_function, optimizer, dataloader, device, log_file, args, train=True):
    epoch_losses = []
...
if os.path.exists(args.log_file):
    print('[Warning] Log file already exists. New logs will be appended to it.')
...
for epoch_idx in range(1, args.num_epochs + 1):
    ...
    checkpoint_path = os.path.join(args.checkpoint_directory, args.checkpoint_prefix + '.%02d.pth' % epoch_idx)
    checkpoint = {
        'args': args,
        'epoch_idx': epoch_idx,
        'model': model.state_dict(),
    ...
    if args.use_validation and validation_loss_history[-1] < min_validation_loss:
        ...
        shutil.copy(checkpoint_path, os.path.join(args.checkpoint_directory, args.checkpoint_prefix + '.best.pth'))

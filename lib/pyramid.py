...
def process_multiscale(image, model, scales=[.5, 1, 2]):
    ...
    all_keypoints = torch.zeros([4, 0])
    all_descriptors = torch.zeros([model.dense_feature_extraction.num_channels, 0])
    
    previous_dense_features = None
    banned = None
    for idx, scale in enumerate(scales):
        current_image = F.interpolate(image, scale_factor=scale, mode='bilinear', align_corners=True)
        ...
        if previous_dense_features is not None:
            dense_features += F.interpolate(previous_dense_features, size=[h, w], mode='bilinear', align_corners=True)
        ...
        valid_displacements = torch.stack([displacements_i[mask], displacements_j[mask]], dim=0).cpu()
        ...
        keypoints = torch.cat([
            ...
            dense_features[0, fmap_pos[0, :], fmap_pos[1, :], fmap_pos[2, :]].cpu().unsqueeze(0) / (idx + 1)
        ], dim=0)
        del fmap_pos

...
from lib.utils import grid_positions, upscale_positions, downscale_positions, savefig, imshow_image
from lib.exceptions import NoGradientError, EmptyTensorError

matplotlib.use('Agg')


def loss_function(model, batch, device, margin=1, safe_radius=4, scaling_steps=3):
    output = model({'image1': batch['image1'].to(device), 'image2': batch['image2'].to(device)})
    ...
    for idx_in_batch in range(batch['image1'].size(0)):
        ...
        try:
            pos1, pos2, ids = warp(pos1, depth1, intrinsics1, pose1, bbox1, depth2, intrinsics2, pose2, bbox2)  # [2, _]
        ...
        if batch['batch_idx'] % batch['log_interval'] == 0:
            ...
            plt.imshow(im1)
            plt.scatter(pos1_aux[1, :], pos1_aux[0, :], s=0.25**2, c=col, marker=',', alpha=0.5)
            plt.axis('off')
            ...
            plt.imshow(im2)
            plt.scatter(pos2_aux[1, :], pos2_aux[0, :], s=0.25**2, c=col, marker=',', alpha=0.5)
            plt.axis('off')
...
def warp(pos1, depth1, intrinsics1, pose1, bbox1, depth2, intrinsics2, pose2, bbox2):
    ...
    X1 = (u1 - intrinsics1[2]) * (Z1 / intrinsics1[0])
    Y1 = (v1 - intrinsics1[5]) * (Z1 / intrinsics1[4])
    ...
    uv2_hom = torch.matmul(intrinsics2.view(3, 3), XYZ2)

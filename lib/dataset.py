...
class MegaDepthDataset(Dataset):
    ...
    def build_dataset(self):
        ...
        for scene in tqdm(self.scenes, total=len(self.scenes)):
            scene_info_path = os.path.join(self.scene_info_path, scene + '.0.npz')
            if not os.path.exists(scene_info_path):
                continue
            scene_info = np.load(scene_info_path)
            valid = np.logical_and(
                np.logical_and(
                    scene_info['overlap_matrix'] >= self.min_overlap_ratio,
                    scene_info['overlap_matrix'] <= self.max_overlap_ratio
                ),
                np.logical_and(
                    scene_info['scale_ratio_matrix'] <= self.max_scale_ratio,
                    scene_info['scale_ratio_matrix'] >= 1. / self.max_scale_ratio
                )
            )
            ...
            for pair_idx in selected_ids:
                ...
                matches = list(points3D_id_to_2D[idx1].keys() & points3D_id_to_2D[idx2].keys())
                ...
                central_match = np.array([point2D1[1] * fact_y1, point2D1[0] * fact_x1, point2D2[1] * fact_y2, point2D2[0] * fact_x2])
                self.dataset.append({
                    'image_path1': image_paths[idx1],
                    'depth_path1': depth_paths[idx1],
                    'intrinsics1': np.array(intrinsics[idx1]),
                    'pose1': poses[idx1],
                    'image_path2': image_paths[idx2],
                    'depth_path2': depth_paths[idx2],
                    'intrinsics2': np.array(intrinsics[idx2]),
                    'pose2': poses[idx2],
    ...
    def recover_pair(self, pair_metadata):
        image1 = imageio.imread(os.path.join(self.base_path, pair_metadata['image_path1']))
        with h5py.File(os.path.join(self.base_path, pair_metadata['depth_path1']), 'r') as hdf5_file:
        ...
        image2 = imageio.imread(os.path.join(self.base_path, pair_metadata['image_path2']))
        with h5py.File(os.path.join(self.base_path, pair_metadata['depth_path2']), 'r') as hdf5_file:
        ...
        return image1, depth1, intrinsics1, pose1, bbox1, image2, depth2, intrinsics2, pose2, bbox2
